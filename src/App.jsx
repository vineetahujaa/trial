...
function IntakeTab({ showToast }) {
  ...
  const loadBatches = async (cursor = null, append = false) => {
    try {
      cursor ? setLoadingMore(true) : setLoading(true);
      const url = cursor ? `/api/batches?limit=25&cursor=${encodeURIComponent(cursor)}` : '/api/batches?limit=25';
      const res = await apiFetch(url);
      const json = await res.json();
      if (!res.ok) throw new Error(json?.error || 'Failed to load batches');
      setBatches((prev) => append ? [...prev, ...(json.items || [])] : (json.items || []));
    ...
    try {
      const res = await apiFetch('/api/batches', {
        ...
        body: JSON.stringify({
          ...
          formFactor: formData.form,
          intakeWeightKg: formData.weight === '' ? null : Number(formData.weight),
          intakeDate: formData.date || null,
        }),
      });
      const json = await res.json();
      ...
      if (json.item) {
        setBatches((prev) => [json.item, ...prev]);
        showToast(`Batch ${json.item.batch_number} created successfully.`);
      }
      setFormData({
        batch: '',
        source: '',
  ...
  const handleCopyBatch = async (batchNumber) => {
    try {
      await navigator.clipboard.writeText(batchNumber);
      showToast(`Copied ${batchNumber}.`);
    } catch {
      showToast('Copy failed.', 'warning');
    }
  };

  const openFilePicker = (batchId = null) => {
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      fd.append('file', file);
      if (uploadBatchId) fd.append('batchId', uploadBatchId);
      ...
      showToast(`Uploaded ${json.file?.original_filename || file.name} successfully.`);
    } catch (error) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm relative overflow-hidden">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="bg-gray-900 hover:bg-gray-800 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-sm transition-colors"
          >
            {showForm ? 'Close Registration' : 'Register Batch'}
          </button>
          <button
            onClick={() => openFilePicker(null)}
            disabled={uploading}
            className="bg-white hover:bg-gray-50 text-gray-700 px-4 py-2 rounded-lg text-sm font-bold shadow-sm transition-colors border border-gray-200 flex items-center gap-2"
          >
            <Download className="w-4 h-4 rotate-180" /> {uploading ? 'Uploading...' : 'Upload Data'}
          </button>
          <input
            ref={fileRef}
            type="file"
      ...
      {showForm && (
        <div className="mb-8 p-6 bg-emerald-50/50 border border-emerald-100 rounded-xl shadow-inner">
          <h4 className="text-sm font-bold text-emerald-800 mb-4">Register Batch</h4>
          <div className="grid grid-cols-1 sm:grid-cols-6 gap-4 items-end">
            <div>
              ...
              <input type="text" value={formData.batch} onChange={e => setFormData({...formData, batch: e.target.value})} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-emerald-500 outline-none font-mono uppercase" placeholder="B-7495" />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-xs font-bold text-gray-600 mb-1">Battery Source</label>
              <input type="text" value={formData.source} onChange={e => setFormData({...formData, source: e.target.value})} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-emerald-500 outline-none" placeholder="Apex Recyclers" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-600 mb-1">Battery Type</label>
              <input type="text" value={formData.type} onChange={e => setFormData({...formData, type: e.target.value})} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-emerald-500 outline-none" placeholder="Lithium-Ion" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-600 mb-1">Form Factor</label>
              <input type="text" value={formData.form} onChange={e => setFormData({...formData, form: e.target.value})} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-emerald-500 outline-none" placeholder="Cylindrical (18650)" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-600 mb-1">Intake Weight</label>
              <input type="number" value={formData.weight} onChange={e => setFormData({...formData, weight: e.target.value})} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-emerald-500 outline-none font-mono" placeholder="1200" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-600 mb-1">Date Registered</label>
              <input type="date" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-emerald-500 outline-none" />
            </div>
            <div className="sm:col-span-2">
              <button onClick={handleCreateBatch} disabled={creating} className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-lg px-4 py-2 transition-colors h-[38px] text-sm shadow-sm w-full">
      ...
      {loading ? (
        <div className="py-10 text-center text-gray-400">Loading intake ledger...</div>
      ) : (
        <>
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm border-collapse">
              ...
              <tbody className="divide-y divide-gray-100 text-gray-700">
                {batches.map((b) => (
                  <tr key={b.id} className="hover:bg-gray-50/50 transition-colors group">
                    <td className="px-6 py-4 font-mono font-bold text-gray-900">
                      <button onClick={() => handleCopyBatch(b.batch_number)} className="underline decoration-dotted hover:text-emerald-700">
                        {b.batch_number}
                      </button>
                    </td>
                    <td className="px-6 py-4 font-medium">{b.source || '—'}</td>
                    <td className="px-6 py-4"><span className="px-2.5 py-1 bg-gray-100 border border-gray-200 rounded text-xs font-mono">{b.battery_type || '—'}</span></td>
                    <td className="px-6 py-4 text-gray-500">{b.form_factor || '—'}</td>
                    <td className="px-6 py-4 font-mono font-semibold text-emerald-600">{b.intake_weight_kg != null ? `${b.intake_weight_kg} kg` : '—'}</td>
                    <td className="px-6 py-4 text-gray-500 text-xs">{b.intake_date || '—'}</td>
                    <td className="px-6 py-4 text-right">
                      <div className="inline-flex items-center gap-2">
                        <button onClick={() => openFilePicker(b.id)} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 border border-gray-200 rounded-lg text-xs font-bold transition-all shadow-sm">
                          <Download className="w-3.5 h-3.5 rotate-180" /> Upload
                        </button>
                        <button onClick={() => showToast(`Sending print job for QR Label ${b.batch_number} to factory printer.`, 'success')} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white text-gray-600 hover:bg-emerald-50 hover:text-emerald-700 border border-gray-200 rounded-lg text-xs font-bold transition-all shadow-sm">
                ...
                {batches.length === 0 && (
                  <tr>
                    <td className="px-6 py-8 text-center text-gray-400" colSpan={7}>No batches found yet.</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          {nextCursor && (
            <div className="pt-6 flex justify-center">
              <button onClick={() => loadBatches(nextCursor, true)} disabled={loadingMore} className="px-6 py-2 rounded-xl border border-gray-200 bg-gray-50 hover:bg-gray-100 font-bold text-sm">
...
function SearchTab({ showToast }) {
  ...
  const loadHistory = async () => {
    try {
      ...
      const [chatRes, incidentRes] = await Promise.all([
        apiFetch('/api/chat?conversationId=plant-ai&limit=80'),
        apiFetch('/api/extract?limit=25'),
      ]);

      const chatJson = await chatRes.json();
      if (chatRes.ok) setMessages(chatJson.messages || []);
      else throw new Error(chatJson?.error || 'Failed to load chat history');

      const incidentJson = await incidentRes.json();
      if (incidentRes.ok) setRecentIncidents(incidentJson.incidents || []);
    } catch (error) {
  ...
  const handleAsk = async () => {
    ...
    try {
      const res = await apiFetch('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ conversationId: 'plant-ai', message: userText }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json?.error || 'Failed to send message');

      setMessages((prev) => {
        const replaced = prev.map((m) => m.id === optimisticId ? json.userMessage : m);
        return [...replaced, json.assistantMessage];
      });
    } catch (error) {
      setMessages((prev) => prev.map((m) => m.id === optimisticId ? { ...m, error: true, optimistic: false } : m));
  ...
  const handleSaveIncident = async () => {
    if (!incidentText.trim() || incidentSaving) return;
    setIncidentSaving(true);
    try {
      const res = await apiFetch('/api/extract', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ text: incidentText.trim() }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json?.error || 'Failed to extract incident');
      setRecentIncidents((prev) => [json.incident, ...prev]);
      setIncidentText('');
      showToast('Incident manually logged and saved to the ledger.', 'success');
    } catch (error) {
      showToast(error.message || 'Failed to extract incident', 'error');
    } finally {
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-[1.2fr_0.8fr] gap-6">
      <div className="bg-white border border-gray-200 rounded-2xl p-8 shadow-sm flex flex-col h-[650px]">
        ...
        <div className="rounded-xl bg-gray-50 border border-gray-200 p-6 min-h-[260px] flex-1 overflow-y-auto whitespace-pre-wrap text-sm text-gray-700 font-medium space-y-4">
          {loadingHistory ? (
            <div className="text-gray-400">Loading chat history...</div>
          ) : (
            <>
              {messages.length === 0 && (
                <div className="text-gray-500">Upload plant files in the operations tab, then ask grounded questions about batches, suppliers, incidents, and performance metrics.</div>
              )}
              {messages.map((m) => (
                <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] px-4 py-3 rounded-2xl shadow-sm text-sm ${m.role === 'user' ? (m.error ? 'bg-red-600 text-white' : 'bg-gray-900 text-white') : 'bg-white border border-gray-200 text-gray-800'}`}>
                    <div className="whitespace-pre-wrap">{m.content}</div>
                    {m.optimistic && <div className="text-[10px] opacity-70 mt-1">Sending...</div>}
                    {m.error && <div className="text-[10px] opacity-80 mt-1">Failed. Please retry.</div>}
                  </div>
                </div>
              ))}
              {loading && (
                <div className="flex justify-start">
                  <div className="bg-white border border-gray-200 text-gray-700 px-4 py-3 rounded-2xl shadow-sm flex items-center gap-2 text-sm">
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-8 shadow-sm flex flex-col h-[340px]">
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-gray-900 text-lg font-bold tracking-tight">Recent Logs</h3>
            <button onClick={loadHistory} className="text-xs font-bold text-gray-500 hover:text-gray-900">Refresh</button>
          </div>
            ...
            {recentIncidents.length === 0 ? (
              ...
              recentIncidents.map((incident) => (
                <div key={incident.id} className="rounded-xl bg-gray-50 border border-gray-200 p-4">
                  ...
                  <div className="text-[10px] font-mono text-gray-400 font-medium">{incident.created_at_ms ? new Date(incident.created_at_ms).toLocaleString() : incident.created_at}</div>
                </div>
              ))
            )}
          </div>
